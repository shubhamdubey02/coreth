...
For more details on Avalanche Warp Messaging, see the CryftGo [Warp README](https://docs.cryft.network/build/cross-chain/awm/deep-dive).
...
The actual `message` is the entire [Avalanche Warp Unsigned Message](https://github.com/shubhamdubey02/cryftgo/blob/master/vms/platformvm/warp/unsigned_message.go#L14) including an [AddressedCall](https://github.com/shubhamdubey02/cryftgo/tree/master/vms/platformvm/warp/payload#readme). The unsigned message is emitted as the unindexed data in the log.
...
The `blockchainID` in Avalanche refers to the txID that created the blockchain on the Avalanche P-Chain ([docs](https://docs.cryft.network/specs/platform-transaction-serialization#unsigned-create-chain-tx)).

### Predicate Encoding

Avalanche Warp Messages are encoded as a signed Avalanche [Warp Message](https://github.com/shubhamdubey02/cryftgo/blob/master/vms/platformvm/warp/message.go) where the [UnsignedMessage](https://github.com/shubhamdubey02/cryftgo/blob/master/vms/platformvm/warp/unsigned_message.go)'s payload includes an [AddressedPayload](https://github.com/shubhamdubey02/cryftgo/blob/master/vms/platformvm/warp/payload/payload.go).
...
Therefore, we use the [Predicate Utils](https://github.com/shubhamdubey02/coreth/blob/master/predicate/Predicate.md) package to encode the actual byte slice of size N into the access list.

...
func TestImportTxVerify(t *testing.T) {
	...
	tests := map[string]atomicTxVerifyTest{
		...
		"inputs sorted incorrectly": {
			generate: func(t *testing.T) UnsignedAtomicTx {
				tx := *importTx
				tx.ImportedInputs = []*cryft.TransferableInput{
					tx.ImportedInputs[1],
					tx.ImportedInputs[0],
				}
				return &tx
		...
		"non-CRYFT input Banff": {
			...
			ctx:         ctx,
			rules:       banffRules,
			expectedErr: errImportNonCRYFTInputBanff.Error(),
		},
		"non-CRYFT output Banff": {
			...
			ctx:         ctx,
			rules:       banffRules,
			expectedErr: errImportNonCRYFTOutputBanff.Error(),
		},
	}
...
func TestNewImportTx(t *testing.T) {
	...
	// createNewImportCRYFTTx adds a UTXO to shared memory and then constructs a new import transaction
	// and checks that it has the correct fee for the base fee that has been used
	createNewImportCRYFTTx := func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) *Tx {
		txID := ids.GenerateTestID()
		_, err := addUTXO(sharedMemory, vm.ctx, txID, 0, vm.ctx.CRYFTAssetID, importAmount, testShortIDAddrs[0])
		if err != nil {
			t.Fatal(err)
		}
		...
		actualCRYFTBurned, err := importTx.Burned(vm.ctx.CRYFTAssetID)
		if err != nil {
			t.Fatal(err)
		}
		...
		if actualCRYFTBurned != actualFee {
			t.Fatalf("CRYFT burned (%d) != actual fee (%d)", actualCRYFTBurned, actualFee)
		}
	...
	checkState := func(t *testing.T, vm *VM) {
		...
		tx := txs[0]
		actualCRYFTBurned, err := tx.UnsignedAtomicTx.Burned(vm.ctx.CRYFTAssetID)
		if err != nil {
			t.Fatal(err)
		}
		...
		expectedRemainingBalance := new(big.Int).Mul(new(big.Int).SetUint64(importAmount-actualCRYFTBurned), x2cRate)
		...
		"simple ANT import": {
			UnsignedImportTx: &UnsignedImportTx{
				...
				ImportedInputs: []*cryft.TransferableInput{
					...
					{
						UTXOID: cryft.UTXOID{TxID: ids.GenerateTestID()},
						Asset:  cryft.Asset{ID: antAssetID},
						In: &secp256k1fx.TransferInput{
							Amt:   importAmount,
							Input: secp256k1fx.Input{SigIndices: []uint32{0}},
						},
...
func TestImportTxSemanticVerify(t *testing.T) {
	tests := map[string]atomicTxTest{
		...
		"UTXO AssetID mismatch": {
			setup: func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) *Tx {
				...
				tx := &Tx{UnsignedAtomicTx: &UnsignedImportTx{
					...
					ImportedInputs: []*cryft.TransferableInput{{
						UTXOID: utxo.UTXOID,
						Asset:  cryft.Asset{ID: vm.ctx.CRYFTAssetID}, // Use a different assetID then the actual UTXO
						In: &secp256k1fx.TransferInput{
							Amt:   1,
							Input: secp256k1fx.Input{SigIndices: []uint32{0}},
						},
		...
		"insufficient CRYFT funds": {
			setup: func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) *Tx {
				txID := ids.GenerateTestID()
				utxo, err := addUTXO(sharedMemory, vm.ctx, txID, 0, vm.ctx.CRYFTAssetID, 1, testShortIDAddrs[0])
				if err != nil {
					t.Fatal(err)
				}
		...
		"non-unique EVM Outputs": {
			setup: func(t *testing.T, vm *VM, sharedMemory *atomic.Memory) *Tx {
				txID := ids.GenerateTestID()
				utxo, err := addUTXO(sharedMemory, vm.ctx, txID, 0, vm.ctx.CRYFTAssetID, 2, testShortIDAddrs[0])
				if err != nil {
					t.Fatal(err)
				}
	...
	tests := map[string]atomicTxTest{
		"CRYFT UTXO": {
			...
			checkState: func(t *testing.T, vm *VM) {
				...
				cryftBalance := sdb.GetBalance(testEthAddrs[0])
				if cryftBalance.Cmp(x2cRate) != 0 {
					t.Fatalf("Expected CRYFT balance to be %d, found balance: %d", x2cRate, cryftBalance)
				}
			},
		},
		"non-CRYFT UTXO": {
			...
			checkState: func(t *testing.T, vm *VM) {
				...
				cryftBalance := sdb.GetBalance(testEthAddrs[0])
				if cryftBalance.Cmp(common.Big0) != 0 {
					t.Fatalf("Expected CRYFT balance to be 0, found balance: %d", cryftBalance)
				}
			},
		},
	}
